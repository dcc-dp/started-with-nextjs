...
export default async function ProdukPage(props) {
  ...
  return (
    <div className="grid grid-cols-6 mt-5 mb-5 gap-5 mx-5 place-items-center">
      ...
      {products.length > 0 &&
        products.map((item) => (
          <div
            key={item.id}
            className="w-full max-w-sm bg-neutral-primary p-6 border border-default rounded-lg shadow-xs"
          >
            <a href="#">
              <img
                className="rounded-base mb-6 object-cover h-50 w-full"
                src={item.image}
                alt="product image"
              />
            </a>
